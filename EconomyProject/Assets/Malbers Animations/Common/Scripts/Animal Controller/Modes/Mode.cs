...
namespace MalbersAnimations.Controller
{
    ...
    public class Mode
    {
        ...
        protected virtual void Activate(int AbilityIndex)
        {
            ...
            {
                affect = ActiveAbility.OverrideProperties.affect;

                if (affect != AffectStates.None && !AffectStates_Empty)
                {
                    if (affect == AffectStates.Exclude && ContainStateOverride(ActiveAbility.OverrideProperties, ID)      //If the new state is on the Exclude State
                || (affect == AffectStates.Inlcude && !ContainStateOverride(ActiveAbility.OverrideProperties, ID)))    //OR If the new state is not on the Include State
                    {
                        return;
                    }
                }
